...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit Tbl Post'), ['action' => 'edit', $dataPost->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Tbl Post'), ['action' => 'delete', $dataPost->id], ['confirm' => __('Are you sure you want to delete # {0}?', $dataPost->id)]) ?> </li>
...
<div class="tblPost view large-9 medium-8 columns content">
    ...
    <table class="vertical-table">
        ...
        <tr>
            ...
            <td><?= $dataPost->has('tbl_user') ? $this->Html->link($dataPost->tbl_user->id, ['controller' => 'TblUser', 'action' => 'view', $dataPost->tbl_user->id]) : '' ?></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td><?= $this->Number->format($dataPost->page_id) ?></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td><?= $this->Number->format($dataPost->create_time) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Update Time') ?></th>
            <td><?= $this->Number->format($dataPost->update_time) ?></td>
        </tr>
    </table>
    <div class="row">
        <h4><?= __('Content') ?></h4>
        <?= $this->Text->autoParagraph(h($dataPost->content)); ?>
    </div>
    <div class="row">
        <h4><?= __('Tags') ?></h4>
        <?= $this->Text->autoParagraph(h($dataPost->tags)); ?>
    </div>
</div>
